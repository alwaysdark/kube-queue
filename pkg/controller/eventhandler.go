...
func (c *Controller) AddQueueUnit(obj interface{}) {
	...
	if !ok {
		klog.Infof("queue is not exist %s", queueName)
		c.multiSchedulingQueue.EnOmissiveQu(unit)
		return
	}
...
func (c *Controller) DeleteQueueUnit(obj interface{}) {
	...
	if !ok {
		klog.Infof("queue is not exist %s", queueName)
		c.multiSchedulingQueue.DeOmissiveQu(unit)
		return
	}
...
func (c *Controller) UpdateQueueUnit(oldObj, newObj interface{}) {
	...
	if !ok {
		klog.Infof("queue is not exist %s", queueName)
		c.multiSchedulingQueue.UpdateOmissiveQu(oldQu, newQu)
		return
	}
